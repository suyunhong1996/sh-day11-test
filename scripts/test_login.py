...
from Base.get_file_data import GetFileData
...
def get_login_data():
    ...
    for i in login_data:
        if login_data.get(i).get('toast'):
            # 预期失败测试用例  case_num,account,passwd,toast,expect_data
            fail_list.append((i, login_data.get(i).get("account"),
                                 login_data.get(i).get("passwd"),
                                 login_data.get(i).get("toast"),
                                 login_data.get(i).get("expect_data")))
        else:
            # 预期成果测试用例 case_num,account,passwd,expect_data
...
class TestLogin:
    ...
    def auto_in_login(self):
        ...
        # 点击已有账号
        self.page_obj.get_signpage().click_sign()
    ...
    def test_login_suc(self, case_num, account, passwd, exp_data):
        ...
        try:
            ...
            try:
                ...
                """停留在个人中心，需要执行退出操作"""
                # 截图
                self.page_obj.get_loginpage().scroll_page()
                assert False
            finally:
                # 点击设置
